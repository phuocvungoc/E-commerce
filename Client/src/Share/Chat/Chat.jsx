...
const socket = io("https://server-ecommerce-nodejs.herokuapp.com", {
...
function Chat(props) {
  ...
  // useEffect(async () => {
  //   if (!roomId) {
  //     const response = await MessengerAPI.newRoom();
  //     setMessage(response);
  //     setRoomId(response._id);
  //     localStorage.setItem("roomId", response._id);
  //   }
  // }, [roomId]);
  ...
  const fetchData = async () => {
    if (roomId) {
      const response = await MessengerAPI.getMessageByRoomId(roomId);
